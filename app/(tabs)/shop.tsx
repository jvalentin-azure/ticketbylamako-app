...
export default function ShopScreen() {
  ...
  useEffect(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter((p) =>
        decodeHtmlEntities(p.name).toLowerCase().includes(q),
      );
    }
    setFiltered(result);
  ...
  const renderProduct = ({ item }: { item: WCProduct }) => (
    ...
    >
      <View style={{ position: "relative" }}>
        ...
        >
          <IconSymbol
            name={isFavorite(item.id, "product") ? "heart.fill" : "heart"}
            size={16}
            color={isFavorite(item.id, "product") ? "#EF4444" : "#fff"}
          />
        </TouchableOpacity>
      </View>
      <View style={styles.productBody}>
        ...
        <Text
          style={[styles.productCat, { color: colors.muted }]}
          numberOfLines={1}
        >
          {item.categories
            ?.filter((c) => c.slug.startsWith("boutique-"))
