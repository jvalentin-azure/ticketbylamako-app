...
export default function NotificationSettingsScreen() {
  ...
  const handleToggle = async (key: keyof NotificationPreferences) => {
    ...
    try {
      ...
      if (key === "eventReminders" && !updated.eventReminders) {
        await cancelScheduledNotificationsByType("event_reminder");
      }
      if (await getStoredPushToken()) {
        const synced = await registerPushTokenWithBackend();
        if (!synced) {
          Alert.alert(
            "Synchronisation en attente",
            "Votre choix est enregistré sur cet appareil. Il sera synchronisé avec votre compte dès que la connexion sera rétablie.",
          );
        }
      }
    } catch {
      await saveNotificationPreferences(previous, user?.id).catch(
        () => undefined,
      );
      setPrefs(previous);
      Alert.alert(
        ...
        { text: "Annuler", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Rappels effacés",
                "Les rappels planifiés ont été supprimés.",
              );
            } catch {
              Alert.alert(
                "Erreur",
                "Impossible d'effacer les rappels planifiés.",
              );
            }
          },
        },
        ...
        {Platform.OS !== "web" && permissionStatus === "granted" && (
          <TouchableOpacity
            ...
            accessibilityLabel={
              hasPushToken
                ? undefined
                : "Réessayer la synchronisation des notifications"
            }
          ...
          >
            {enabling ? (
              ...
              <IconSymbol
                name={
                  hasPushToken ? "checkmark.circle.fill" : "arrow.clockwise"
                }
                size={24}
                color="#22C55E"
              />
            ...
            SETTINGS.map((setting, i) => (
              ...
              >
                ...
                <Switch
                  ...
                  thumbColor={
                    prefs[setting.key] ? colors.primary : colors.muted
                  }
                />
              </View>
            ))}
        ...
        >
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              ...
              <Text style={[styles.settingDesc, { color: colors.muted }]}>
                Annuler tous les rappels d'événements
              </Text>
            </View>
