...
## 2026-05-20 - Client/Back-office API Reconciliation

- [x] Merge client-facing Tickera checkout field routes with back-office ended-event safeguards.
- [x] Add server-side ended-event rejection for native ticket checkout and seating sessions.
- [x] Add mobile order channel metadata for native and seating checkout orders.
- [x] Make event ticket Rewards eligibility opt-in on the v2 catalog payload.
- [x] Read `salesClosed`, `purchasable`, and `ticketingStatus` in native event, cart, and checkout flows.
